  ...
  return new Promise((resolve) => {
    ...
    const walkStep = () => { 
      if (currentPos < stopPos) {
        ...
        setTimeout(walkStep, STEP_INTERVAL_MS); // creates a delay between each step of the cat's walk
      } else {
        resolve();
      }
    };
...
function catWalk() {
  ...
  const catWalkAndDance = () => {
    walk(img, startPos, centerPos)
      ...
      .then(catWalkAndDance); // calls itself to repeat the entire sequence
  };
